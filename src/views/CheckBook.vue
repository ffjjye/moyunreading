...
export default {
  ...
  methods: {
    async initCheckBook() {
      ...
      if (!bookId) {
        console.error('Book ID is missing in route params.');
        return;
      }
      try {
        ...
        await this.$store.dispatch('fetchReviewsForBook', bookId);
        this.bookReviewList = this.currentBookReviews;
