  ...
  methods: {
    writeReview() {
      ...
      this.$http({
        ...
        withCredentials: true // Ensure credentials are sent for session tracking
      })
        .then((res) => {
          ...
          path: "/Book/CheckBook/"+this.$route.params.id,
            ...
            // If backend returns a message other than '评论成功' but no error, clear form and show message
            alert(res.data.message);
            this.clear();
          }
        })
        .catch((err) => {
          console.error('书评发布失败:', err);
          alert("书评发布失败，请稍后重试");
        });
    },
    clear() {
