...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.blogDT = this.$store.state.updates.map(update => ({
        blogID: update.id,
        blogTitle: update.content.substring(0, 30) + '...',
        content: update.content,
        authorID: update.userId,
        recentFinishTime: update.createTime,
        author: update.username,
        authorImage: this.$store.state.person.avatar
      }));

      this.bookReviewDT = this.$store.state.reviews.map(review => ({
      ...
      // 设置用户数据
      this.blogDTUsers = this.blogDT.map(blog => ({
        username: blog.author,
        headImage: blog.authorImage
      }));

      this.bookReviewDTUsers = this.bookReviewDT.map(review => ({
        username: review.author,
        headImage: review.authorImage
      }));
    },
    changeToBook() {
      this.Bookflag = true;
